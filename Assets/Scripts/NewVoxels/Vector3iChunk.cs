...
public struct Vector3iChunk
{
    ...
    public int X
    {
        ...
        set { Binary = (ushort)(Binary & ~0x1F | Mathf.Clamp(value, 0, 31) << 0 & 0x1F); }
    }

    public int Y
    {
        get { return (Binary & 0x3E0) >> 5; }
        set { Binary = (ushort)(Binary & ~0x3E0 | Mathf.Clamp(value, 0, 31) << 5 & 0x3E0); }
    }

    public int Z
    {
        get { return (Binary & 0x7C00) >> 10; }
        set { Binary = (ushort)(Binary & ~0x7C00 | Mathf.Clamp(value, 0, 31) << 10 & 0x7C00); }
    }
    ...
    public static implicit operator Vector3iChunk(WorldPos v)
    {
        return new Vector3iChunk(v.x, v.y, v.z);
    }

    public static implicit operator WorldPos(Vector3iChunk c)
    {
        return new WorldPos(c.X, c.Y, c.Z);
    }
}
