...
public class Frustrum {
    ...
    public void CalculateFrustum() {
        ...
        for (int i = 0; i < 4; i++) {
            Vector4 row = projMatrix.GetRow(i);
            proj[i] = row.x;
            proj[i + 1] = row.y;
            proj[i + 2] = row.z;
            proj[i + 3] = row.z;
        ...
        Matrix4x4 modlMatrix = GL.modelview;
        for (int i = 0; i < 4; i++)
        {
            Vector4 row = modlMatrix.GetRow(i);
            modl[i] = row.x;
            modl[i + 1] = row.y;
            modl[i + 2] = row.z;
            modl[i + 3] = row.z;
        ...
        NormalizePlane(m_Frustum, (int)FrustumSide.Right);
        ...
        NormalizePlane(m_Frustum, (int)FrustumSide.Left);
        ...
        NormalizePlane(m_Frustum, (int)FrustumSide.Bottom);
        ...
        NormalizePlane(m_Frustum, (int)FrustumSide.Top);
        ...
        NormalizePlane(m_Frustum, (int)FrustumSide.Back);
        ...
        NormalizePlane(m_Frustum, (int)FrustumSide.Front);
    }
    ...
    public bool PointInFrustum(float x, float y, float z) {
        ...
        for (int i = 0; i < 6; i++)
        {
            ...
            if (
                m_Frustum[i][(int)PlaneData.A] * x 
                + m_Frustum[i][(int)PlaneData.B] * y 
                + m_Frustum[i][(int)PlaneData.C] * z 
                + m_Frustum[i][(int)PlaneData.D] 
                <= 0)
            {
    ...
    public bool CubeInFrustum(float x, float y, float z, float size) {
        ...
        {
            if (
                  m_Frustum[i][(int)PlaneData.A] * (x - size) 
                + m_Frustum[i][(int)PlaneData.B] * (y - size) 
                + m_Frustum[i][(int)PlaneData.C] * (z - size) 
                + m_Frustum[i][(int)PlaneData.D] 
                > 0)
                continue;
            if (
    ...
    void NormalizePlane(float[][] frustum, int side)
    {
        ...
        float magnitude = (float)Mathf.Sqrt(frustum[side][(int)PlaneData.A] * frustum[side][(int)PlaneData.A] +
